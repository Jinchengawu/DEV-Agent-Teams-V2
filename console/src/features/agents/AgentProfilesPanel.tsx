...
export function AgentProfilesPanel() {
  ...
  return <section className="panel profiles-panel">
    ...
    <div className="profile-workbench"><div className="profile-list">
      {items.map((profile) => <button key={profile.id} className={selectedId === profile.id ? "selected" : ""} onClick={() => setSelectedId(profile.id)}><b>{profile.name}</b><small>{profile.id} · 已发布 Revision {profile.latest_revision ?? "无"}</small></button>)}
      {items.length === 0 && <EmptyState title="尚未创建角色" detail="使用右侧中文表单创建前端、测试、PM 或其他逻辑角色。"/>}
    </div><div className="compact-form profile-editor">
      <h3>创建智能体角色</h3>
      <div className="field-grid"><label>角色 ID<input value={spec.id} disabled={Boolean(selectedId)} onChange={(event) => setSpec({ ...spec, id: event.target.value })}/></label><label>角色名称<input value={spec.name} onChange={(event) => setSpec({ ...spec, name: event.target.value })}/></label></div>
      ...
      <div className="row-actions">{!selectedId && <button className="primary button-icon" disabled={!spec.id || !spec.name || create.isPending} onClick={() => create.mutate()}><Plus size={15}/>创建角色草稿</button>}{selectedId && <><button className="secondary button-icon" disabled={!selectedDraft.data || save.isPending} onClick={() => save.mutate()}><Save size={15}/>保存草稿</button><button className="secondary button-icon" disabled={!selectedDraft.data || validate.isPending} onClick={() => validate.mutate()}><CheckCircle2 size={15}/>校验当前版本</button><button className="primary button-icon" disabled={selectedDraft.data?.validation_status !== "valid" || publish.isPending} onClick={() => publish.mutate()}><Upload size={15}/>发布不可变 Revision</button><button onClick={() => { setSelectedId(undefined); setSpec(defaultSpec()); }}>创建新角色</button></>}</div>
      {selectedDraft.isLoading && <LoadingState label="正在读取角色草稿…"/>}{selectedDraft.data && <small className="field-help">草稿版本 {selectedDraft.data.version} · 校验状态 {validationLabel(selectedDraft.data.validation_status)}</small>}{mutationError && <ErrorState error={mutationError}/>} 
    </div></div>
  </section>;
}
