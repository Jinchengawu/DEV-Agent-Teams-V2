  ...
  return <section className="panel agent-deployments-panel">
    ...
    <div className="deployment-workbench"><div className="deployment-list">
      {items.map((item) => {
        ...
        return <article key={item.id} className="binding-card"><div><span className="eyebrow">{item.provider_id}</span><h3>{item.name}</h3><p>{item.profile_id} · R{item.profile_revision} → {item.instance_id}</p></div><StatusBadge value={item.enabled ? "ready" : item.qualification_status === "failed" ? "failed" : "unknown"}/><dl><dt>资格状态</dt><dd>{qualificationLabel(item.qualification_status)}</dd><dt>隔离</dt><dd>{item.isolation_mode === "dedicated" ? "独占实例" : "共享实例、会话隔离"}</dd><dt>绑定哈希</dt><dd><code>{item.provider_fingerprint.slice(0, 16)}</code></dd></dl>{item.qualification_errors.length > 0 && <div className="validation-errors">{item.qualification_errors.map((error) => <span key={error}>{error}</span>)}</div>}<div className="row-actions">{snapshotStale && <Button icon={<RefreshCw size={14}/>} disabled={refreshSnapshot.isPending} onClick={() => refreshSnapshot.mutate(item)}>刷新快照并重新检查</Button>}<Button icon={<CheckCircle2 size={14}/>} disabled={refreshSnapshot.isPending} onClick={() => command.mutate({ item, action: "qualify" })}>资格检查</Button><Button danger={item.enabled} icon={item.enabled ? <Power size={14}/> : <Play size={14}/>} disabled={item.qualification_status !== "qualified" || refreshSnapshot.isPending} onClick={() => item.enabled ? setPendingDisable(item) : command.mutate({ item, action: "enable" })}>{item.enabled ? "禁用新运行" : "启用部署"}</Button></div></article>;
      })}
      {items.length === 0 && <EmptyState title="尚未创建 Agent 部署" detail="先发布角色并完成运行实例健康检查，再创建可执行部署。"/>}
    </div><div className="compact-form deployment-create"><h3>创建 Deployment</h3><label>Deployment ID<Input value={id} onChange={(event) => setId(event.target.value)} placeholder="例如 frontend-codex"/></label><label>中文名称<Input value={name} onChange={(event) => setName(event.target.value)} placeholder="例如 前端 Codex 部署"/></label><label>已发布 Agent 角色<Select aria-label="已发布 Agent 角色" value={profileId || undefined} placeholder="请选择" onChange={setProfileId} options={publishedProfiles.map((item) => ({ value: item.id, label: `${item.name} · R${item.latest_revision}` }))}/></label><label>已就绪运行实例<Select aria-label="已就绪运行实例" value={instanceId || undefined} placeholder="请选择" onChange={setInstanceId} options={readyInstances.map((item) => ({ value: item.id, label: `${item.name} · ${item.runtime_type}` }))}/></label><label>ACWM Provider<Select aria-label="ACWM Provider" value={providerId || undefined} placeholder="请选择" onChange={setProviderId} options={(providers.data ?? []).map((item) => ({ value: item.id, label: `${item.id} · ${item.capabilities.map((capability) => capability.id).join("、")}` }))}/></label><p className="field-help">创建后必须通过资格检查并显式启用，才会出现在编排节点的可选列表中。</p><Button type="primary" icon={<Link2 size={15}/>} disabled={!id || !name || !profileId || !instanceId || !providerId || create.isPending} onClick={() => create.mutate()}>创建部署</Button>{(create.error || command.error || refreshSnapshot.error) && <ErrorState error={(create.error || command.error || refreshSnapshot.error)!}/>}</div></div>
