...
import { Bot, CheckCircle2, Code2, GitBranch, Maximize2, Plus, Save, ShieldQuestion, Trash2, Workflow, X } from "lucide-react";
import { z } from "zod";
...
export function OrchestrationPage() {
  ...
  const [pendingDelete, setPendingDelete] = useState<{ kind: "node" | "edge"; id: string }>();
  ...
  const removeSelected = () => {
    ...
    const result = deleteGraphNode(semanticNodes, semanticEdges(edges), agentAssignments, selectedNodeId);
    setSemanticNodes(result.nodes);
    setNodes((current) => current.filter((item) => item.id !== selectedNodeId));
    setEdges(createFlowEdges(result.edges));
  ...
  return <div className="pipeline-orchestration">
    ...
    <section className="orchestration-main panel"><div className="panel-head"><span>{selectedPipeline?.name ?? "创建第一条流水线"}</span><small>{draft ? `草稿 V${draft.version} · ${draft.validation_status}` : "无草稿"}</small></div>
      {!selectedPipeline ? <div className="state-box"><div><Workflow size={24}/><b>还没有流水线</b><span>在左侧创建一条流水线后开始 DAG 编排。</span></div></div> : drafts.isLoading ? <LoadingState label="正在加载流水线草稿…"/> : !draft ? <div className="state-box"><b>该流水线没有可编辑草稿</b></div> : <><div className="orchestration-toolbar"><div className="node-forge" aria-label="新增图节点"><button onClick={() => addNode("role")}><Bot size={15}/>角色 Stage</button><button onClick={() => addNode("code")}><Code2 size={15}/>交付 Stage</button><button onClick={() => addNode("gate")}><ShieldQuestion size={15}/>审批 Gate</button><button onClick={() => addNode("loop")}><GitBranch size={15}/>有限 LOOP</button></div><span className="toolbar-separator"/><button onClick={() => save.mutate()}><Save size={15}/>保存图与布局</button><button onClick={() => validate.mutate()}><CheckCircle2 size={15}/>ACWM 图校验</button><button className="primary" disabled={draft.validation_status !== "valid"} onClick={() => publish.mutate()}>发布不可变版本</button>{publish.data && <button onClick={() => activate.mutate(publish.data)}>激活 R{publish.data.revision}</button>}</div>
        ...
        <div className="flow"><ReactFlow nodes={nodes} edges={edges} fitView onInit={setFlowInstance} nodesDraggable nodesConnectable onNodeClick={(_, node) => { setSelectedNodeId(node.id); setSelectedEdgeId(undefined); }} onEdgeClick={(_, edge) => { setSelectedEdgeId(edge.id); setSelectedNodeId(undefined); }} onNodesChange={(changes) => onNodesChange(changes.filter((change) => change.type !== "remove"))} onEdgesChange={(changes) => onEdgesChange(changes.filter((change) => change.type !== "remove"))} onConnect={connect} deleteKeyCode={null}><Background gap={24}/><Controls/><MiniMap nodeColor={(node) => node.className?.toString().includes("loop") ? "#b875ff" : node.className?.toString().includes("gate") ? "#ff9b61" : "#38a3ff"}/></ReactFlow></div>{connectionError && <div className="validation-errors"><b>连线被拒绝</b><span>{connectionError}</span></div>}{draft.validation_errors.length > 0 && <div className="validation-errors"><b>ACWM 校验失败</b>{draft.validation_errors.map((error) => <span key={error}>{error}</span>)}</div>}<div className="revision-strip"><span>节点</span><code>{semanticNodes.length}</code><span>语义边</span><code>{edges.length}</code><span>活动图指纹</span><code>{revision.data?.fingerprint ?? "尚无活动版本"}</code></div>{revision.data && <PublishedBindingSnapshot revision={revision.data}/>}</>}{operationError && <ErrorState error={operationError}/>} </section>
    <aside className="orchestration-inspector panel"><div className="panel-head"><span>图语义检查器</span><small>{selectedNode?.id ?? selectedEdge?.id ?? "请选择节点或边"}</small></div>{selectedNode ? <GraphNodeInspector node={selectedNode} onChange={updateSelected} onDelete={() => setPendingDelete({ kind: "node", id: selectedNode.id })} assignments={agentAssignments} deployments={deployments.data ?? []} providers={providers.data ?? []} onAssignment={(site, deploymentId) => setAgentAssignments((current) => { const next = { ...current }; if (deploymentId) next[site] = deploymentId; else delete next[site]; return next; })}/> : selectedEdge ? <GraphEdgeInspector edge={selectedEdge} onConditionChange={(condition) => setEdges((current) => setEdgeCondition(current, selectedEdge.id, condition))} onDelete={() => setPendingDelete({ kind: "edge", id: selectedEdge.id })}/> : <div className="inspector-placeholder"><GitBranch size={24}/><b>选择节点或连线</b><span>节点坐标只控制布局；连线与条件共同定义执行依赖。</span></div>}</aside>
    <ConfirmDialog open={Boolean(pendingDelete)} title={pendingDelete?.kind === "node" ? `删除节点“${pendingDelete.id}”` : `删除连线“${pendingDelete?.id ?? ""}”`} detail={pendingDelete?.kind === "node" ? "该节点、所有关联依赖边以及对应的 Agent Assignment 将同时从草稿中移除。保存前仍可通过重新加载草稿放弃本次修改。" : "该依赖关系及分支条件将从当前草稿中移除。保存后需要重新建立连线才能恢复。"} confirmLabel={pendingDelete?.kind === "node" ? "确认删除节点" : "确认删除连线"} tone="danger" onCancel={() => setPendingDelete(undefined)} onConfirm={() => { if (pendingDelete?.kind === "node") removeSelected(); else if (pendingDelete?.kind === "edge") { setEdges((current) => current.filter((item) => item.id !== pendingDelete.id)); setSelectedEdgeId(undefined); } setPendingDelete(undefined); }}/>
  </div>;
}
...
function GraphNodeInspector({ node, onChange, onDelete, assignments, deployments, providers, onAssignment }: { node: GraphNode; onChange: (node: GraphNode) => void; onDelete: () => void; assignments: Record<string, string>; deployments: Deployment[]; providers: Provider[]; onAssignment: (site: string, deploymentId: string) => void }) {
  if (node.kind === "loop") return <LoopNodeInspector node={node} onChange={onChange} onDelete={onDelete} assignments={assignments} deployments={deployments} providers={providers} onAssignment={onAssignment}/>;
...
function LoopNodeInspector({ node, onChange, onDelete, assignments, deployments, providers, onAssignment }: { node: LoopNode; onChange: (node: LoopNode) => void; onDelete: () => void; assignments: Record<string, string>; deployments: Deployment[]; providers: Provider[]; onAssignment: (site: string, deploymentId: string) => void }) {
  const [workspaceOpen, setWorkspaceOpen] = useState(false);
  return <div className="step-editor"><StatusBadge value="executing"/><h2>{node.id}</h2><dl><dt>节点类型</dt><dd>有限 LOOP 子图</dd><dt>内部节点</dt><dd>{node.nodes.length}</dd><dt>依赖边</dt><dd>{node.edges.length}</dd></dl><p className="field-hint">循环策略与内部 DAG 在专用工作区编辑，避免在狭窄检查器中误操作。</p><button className="primary button-icon" onClick={() => setWorkspaceOpen(true)}><Maximize2 size={15}/>打开 LOOP 全屏工作区</button><button className="danger button-icon" onClick={onDelete}><Trash2 size={15}/>删除 LOOP</button>{workspaceOpen && <div className="loop-workspace-backdrop" role="presentation"><section className="loop-workspace-dialog" role="dialog" aria-modal="true" aria-labelledby="loop-workspace-title"><header><div><span className="eyebrow">有界循环工作区</span><h2 id="loop-workspace-title">{node.id}</h2><p>策略定义退出边界；内部节点和依赖边定义每轮实际执行顺序。</p></div><button className="secondary button-icon" aria-label="关闭 LOOP 工作区" onClick={() => setWorkspaceOpen(false)}><X size={18}/>关闭</button></header><div className="loop-workspace-content"><aside><label>退出条件策略<input value={node.policy.exit_condition} onChange={(event) => onChange({ ...node, policy: { ...node.policy, exit_condition: event.target.value } })}/></label><label>最大轮次<input type="number" min={1} max={100} value={node.policy.max_iterations} onChange={(event) => onChange({ ...node, policy: { ...node.policy, max_iterations: Number(event.target.value) } })}/></label><label>总超时（秒）<input type="number" min={1} value={node.policy.timeout_seconds} onChange={(event) => onChange({ ...node, policy: { ...node.policy, timeout_seconds: Number(event.target.value) } })}/></label><label>耗尽动作<select value={node.policy.on_exhausted} onChange={(event) => onChange({ ...node, policy: { ...node.policy, on_exhausted: event.target.value as LoopNode["policy"]["on_exhausted"] } })}><option value="fail">失败</option><option value="needs_attention">转人工处理</option><option value="continue">继续下游</option></select></label></aside><LoopBodyEditor loop={node} onChange={onChange} assignments={assignments} deployments={deployments} providers={providers} onAssignment={onAssignment}/></div></section></div>}</div>;
}
...
export function deleteGraphNode(nodes: GraphNode[], edges: SemanticEdge[], assignments: Record<string, string>, nodeId: string): { nodes: GraphNode[]; edges: SemanticEdge[]; assignments: Record<string, string> } {
  return {
    nodes: nodes.filter((node) => node.id !== nodeId),
    edges: edges.filter((edge) => edge.source !== nodeId && edge.target !== nodeId),
    assignments: Object.fromEntries(Object.entries(assignments).filter(([site]) => !site.startsWith(`${nodeId}.`) && !site.startsWith(`${nodeId}/`))),
  };
}
