  ...
  return <div className="delivery-home">
    ...
    <div className="workbench-grid">
      ...
      <form className="delivery-composer surface-card" onSubmit={review} noValidate>
        ...
        <label className="field"><span>交付目标</span><textarea id="delivery-goal" value={requestText} aria-invalid={Boolean(problem && !requestText.trim())} onChange={(event) => { setRequestText(event.target.value); setReviewing(false); setProblem(""); }} placeholder="描述要交付的结果、边界与验收要求"/></label>
        ...
        {!reviewing ? <div className="composer-actions"><span>提交后先检查边界，再创建真实交付运行。</span><button className="primary" type="submit" disabled={pipelines.isLoading || activePipelines.length === 0}>生成交付计划</button></div> :
          <section className="delivery-confirmation" aria-label="确认交付边界">
            ...
            <div className="confirm-actions"><button className="secondary" type="button" onClick={() => { setReviewing(false); document.getElementById("delivery-goal")?.focus(); }}>继续编辑</button><button className="primary" type="button" disabled={create.isPending} onClick={() => create.mutate({ userRequest: requestText.trim(), pipelineRevisionId })}>{create.isPending ? "正在创建交付…" : "确认并启动"}</button></div>
          </section>}
    ...
    <section className="recent-runs">
      <div className="section-head"><div><p className="eyebrow">运行上下文</p><h2>最近运行</h2><p>继续最近一次交付，不重新寻找分散页面。</p></div><button className="quiet-button" type="button" onClick={() => deliveries.refetch()}><RefreshCw size={15}/>刷新</button></div>
