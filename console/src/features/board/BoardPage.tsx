...
export function BoardPage() {
  ...
  return <>
    <div className="board-toolbar"><div><span className="eyebrow">项目事件投影 · {projectId}</span><b>拖动卡片只发出合法命令</b></div><div className="board-filters"><label className="search-field"><Search size={15}/><input aria-label="搜索看板任务" placeholder="搜索任务、交付或验收 ID" value={query} onChange={(event) => setQuery(event.target.value)}/></label><label>状态列<select aria-label="筛选状态列" value={columnFilter} onChange={(event) => setColumnFilter(event.target.value as BoardColumn | "all")}><option value="all">全部状态</option>{columns.map((column) => <option key={column.id} value={column.id}>{column.label}</option>)}</select></label><span>{visibleItems.length}/{board.data?.length ?? 0} 个任务</span></div></div>
    ...
    {selected && <aside className="work-item-drawer" role="dialog" aria-modal="false" aria-labelledby="work-item-title"><header><div><span className="eyebrow">项目任务详情</span><h2 id="work-item-title">{selected.title}</h2></div><button className="secondary" aria-label="关闭任务详情" onClick={() => setSelected(undefined)}><X size={16}/></button></header><dl><dt>交付 ID</dt><dd><code>{selected.delivery_id}</code></dd><dt>当前状态</dt><dd><StatusBadge value={selected.column}/></dd><dt>投影版本</dt><dd>v{selected.version}</dd><dt>验收标准</dt><dd>{selected.acceptance_ids.length ? selected.acceptance_ids.join(" · ") : "等待任务合同"}</dd><dt>可用命令</dt><dd>{selected.available_commands.length ? selected.available_commands.map(commandLabel).join("、") : "当前无可用命令"}</dd></dl><Link className="primary drawer-link" to={`${projectPath(projectId, "deliveries")}?delivery_id=${encodeURIComponent(selected.delivery_id)}`}>打开该交付并审查证据</Link></aside>}
  </>;
}
...
function WorkCard({ item, overlay = false, onOpen }: { item: WorkItem; overlay?: boolean; onOpen?: () => void }) {
  ...
  return <article ref={setNodeRef} style={{ transform: CSS.Translate.toString(transform) }} className={`work-card ${overlay ? "overlay" : ""} ${isDragging ? "dragging" : ""}`}><div className="card-top"><StatusBadge value={item.column}/><div><button className="card-detail-button" aria-label={`查看任务 ${item.title}`} onClick={onOpen} disabled={!onOpen}><Eye size={15}/></button><button className="drag-handle" aria-label={`拖动交付 ${item.id}`} {...listeners} {...attributes} disabled={item.available_commands.length === 0}><GripVertical size={16}/></button></div></div><small>交付 {item.delivery_id.slice(0, 8)}</small><h3>{item.title}</h3><p>{item.acceptance_ids.join(" · ") || "等待任务合同"}</p><div className="command-hints">{item.available_commands.length ? item.available_commands.map((value) => <span key={value}>{commandLabel(value)}</span>) : <span>当前无可用命令</span>}</div></article>;
}
