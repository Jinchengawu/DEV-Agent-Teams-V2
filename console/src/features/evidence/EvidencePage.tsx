...
export function EvidencePage() {
  ...
  return <div className="evidence-workbench">
    <section className="panel evidence-ledger">
      ...
      <div className="filter-bar"><Filter size={16}/><Input aria-label="按交付筛选" placeholder="交付 ID" value={deliveryFilter} onChange={(event) => setDeliveryFilter(event.target.value)}/><Select aria-label="按证据类型筛选" value={kindFilter} onChange={setKindFilter} options={kinds.map((kind) => ({ value: kind, label: kind ? artifactTypeLabel(kind) : "全部类型" }))}/><Select aria-label="按完整性筛选" value={statusFilter} onChange={setStatusFilter} options={[{ value: "", label: "全部完整性" }, { value: "verified", label: "已验证" }, { value: "invalid", label: "无效" }, { value: "unavailable", label: "不可用" }]}/></div>
      {filtered.length === 0 ? <EmptyState title="没有符合条件的真实证据" detail="清除筛选，或先完成交付阶段。系统不会补造缺失证据。"/> : <div className="ledger-table" role="table">{filtered.map((item) => <Button type="text" block key={item.id} className={selected?.id === item.id ? "selected" : ""} onClick={() => setSelected(item)}><StatusBadge value={item.status}/><span><b>{artifactTypeLabel(item.kind)}</b><small>{item.delivery_id} · {item.producer_identity}</small></span><code>{item.content_sha256?.slice(0, 16) ?? "无哈希"}</code></Button>)}</div>}
    </section>
    <section className="panel evidence-inspector">
      ...
      {!selected ? <EmptyState title="选择一条证据" detail="这里会显示来源、内容哈希、验证结果和原始结构化载荷。"/> : <>
        ...
        <Collapse className="evidence-payload" defaultActiveKey={["payload"]} items={[{ key: "payload", label: "查看结构化载荷", children: <pre className="payload-view">{JSON.stringify(selected.payload, null, 2)}</pre> }]}/>
        <div className="row-actions"><Button icon={<ShieldCheck size={16}/>} disabled={verify.isPending} onClick={() => verify.mutate(selected.id)}>重新验证当前证据</Button><Button icon={<Copy size={16}/>} disabled={!selected.content_sha256} onClick={async () => {
        ...
        }}>复制内容哈希</Button><Button icon={<Download size={16}/>} onClick={() => downloadEvidence(selected)}>导出证据 JSON</Button></div>
