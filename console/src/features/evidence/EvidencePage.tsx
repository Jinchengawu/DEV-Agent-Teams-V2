...
export function EvidencePage() {
  ...
  const evidence = useQuery({ queryKey: ["evidence", projectId], queryFn: () => request<EvidenceRecord[]>(`/v1/evidence?project_id=${encodeURIComponent(projectId)}`), refetchInterval: 2500 });
  const verify = useMutation({ mutationFn: (id: string) => request<EvidenceRecord>(`/v1/evidence/${id}/verify`, { method: "POST" }), onSuccess: async (record) => { setSelected(record); await client.invalidateQueries({ queryKey: ["evidence", projectId] }); } });
