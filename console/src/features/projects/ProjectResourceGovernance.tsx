    ...
    <div className="project-resource-grid">
      <article>
        ...
        {(pipelines.data ?? []).filter((pipeline) => pipeline.active_revision !== null).length === 0 ? <EmptyState title="没有可授权流水线" detail="请先在可视化编排中发布并激活版本。"/> : <div className="resource-access-list">{(pipelines.data ?? []).filter((pipeline) => pipeline.active_revision !== null).map((pipeline) => {
          ...
          return <div key={`${pipeline.id}:${revision}`}><div><b>{pipeline.name}</b><small><code>{pipeline.id}:R{revision}</code></small></div><StatusBadge value={binding?.is_default && enabled ? "default" : enabled ? "enabled" : "disabled"}/><div className="resource-actions">{!enabled ? <Button disabled={!mutable || updatePipeline.isPending} onClick={() => updatePipeline.mutate({ pipeline_revision_id: `${pipeline.id}:${revision}`, enabled: true, is_default: !defaultBinding, expected_version: binding?.version })}>授权</Button> : <><Button disabled={!mutable || binding?.is_default || updatePipeline.isPending} onClick={() => updatePipeline.mutate({ pipeline_revision_id: `${pipeline.id}:${revision}`, enabled: true, is_default: true, expected_version: binding?.version })}>设为默认</Button><Button danger title={binding?.is_default ? "请先将另一条流水线设为默认" : undefined} disabled={!mutable || binding?.is_default || updatePipeline.isPending} onClick={() => updatePipeline.mutate({ pipeline_revision_id: `${pipeline.id}:${revision}`, enabled: false, is_default: false, expected_version: binding?.version })}>停用</Button></>}</div></div>;
        })}</div>}
      ...
      <article>
        ...
        {(deployments.data ?? []).length === 0 ? <EmptyState title="没有 Agent 部署" detail="请先在智能体实例中发布角色并完成部署资格检查。"/> : <div className="resource-access-list">{(deployments.data ?? []).map((deployment) => {
          ...
          return <div key={deployment.id}><div><b>{deployment.name}</b><small>{deployment.profile_id} · {deployment.adapter_id}</small></div><StatusBadge value={enabled ? "enabled" : usable ? "qualified" : deployment.qualification_status}/><div className="resource-actions"><Button danger={enabled} disabled={!mutable || updateDeployment.isPending || (!enabled && !usable)} title={!enabled && !usable ? "部署必须先启用并通过资格检查" : undefined} onClick={() => updateDeployment.mutate({ deployment_id: deployment.id, enabled: !enabled, expected_version: access?.version })}>{enabled ? "撤销授权" : "授权使用"}</Button></div></div>;
        })}</div>}
