...
from agent_team_os.infrastructure.verification.fullstack import verify_fullstack
...
def test_qa_contract_failure_log_names_missing_stable_case_ids(
    ...
    async def passing_but_incomplete_qa(command, **_kwargs):
        Path(command[6]).write_text(
            ...
            '"test_health_e2e.HealthE2E.test_other"]}'
        )
    ...
    status, raw_report = asyncio.run(
        verify_fullstack(
            ...
            sources=sources,
            store=store,
            run_command=passing_but_incomplete_qa,
    ...
    assert "test_invalid_response" in log
